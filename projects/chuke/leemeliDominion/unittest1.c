...
int main() {
    ...
    for(i = 0; i < state.discardCount[thisPlayer]; ++i) {
        if(state.discard[thisPlayer][i] == estate) {
            ++stateNumEstates;
        }
    }
    for(i = 0; i < test.discardCount[thisPlayer]; ++i) {
        if(test.discard[thisPlayer][i] == estate) {
            ++testNumEstates;
        }
    }
